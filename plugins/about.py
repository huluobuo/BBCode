...
class AboutDialog(CommonDialogEx):
    def __init__(self, master):
        ...
        heading_label = ttk.Label(
            self.main_frame, text="BBCode " + thonny.get_version(), font=heading_font
        )
        heading_label.grid(pady=(self.get_large_padding(), self.get_small_padding()))

        # Subtitle
        subtitle_label = ttk.Label(
            self.main_frame, text="基于 Thonny 的 Python IDE", font=("Microsoft YaHei UI", 10)
        )
        subtitle_label.grid()
...
def load_plugin() -> None:
    ...
    get_workbench().add_command("about", "help", tr("About BBCode"), open_about, group=61)
