...
class Workbench(tk.Tk):
        ...
        for class_ in self._language_server_proxy_classes:
            ...
            ls_proxy = class_(
                InitializeParams(
                    ...
                    processId=os.getpid(),
                    clientInfo=ClientInfo(name="BBCode", version=thonny.get_version()),
    ...
    def _init_icon(self) -> None:
        # Window icons - 使用 BBCode 图标
        if running_on_linux() and ui_utils.get_tk_version_info() >= (8, 6):
            self.iconphoto(True, self.get_image("bbc.png"))
        else:
            # Windows 使用 bbc.png 作为图标
            try:
                self.iconphoto(True, self.get_image("bbc.png"))
            except Exception:
                # 如果失败，尝试使用 ico 文件
    ...
    def _load_plugins_from_path(self, path: List[str], prefix: str) -> None:
        ...
        for _, module_name, _ in sorted(pkgutil.iter_modules(path, prefix), key=lambda x: x[2]):
            ...
            elif module_name in DISABLED_PLUGINS:
                logger.debug("Skipping disabled plug-in %s", module_name)
            else:
                try:
    ...
    def _check_version_alignment(self):
        ...
        if installation_version != embedded_version:
            messagebox.showwarning(
                "Warning",
                f"BBCode's installation version is reported as {installation_version!r},\n"
